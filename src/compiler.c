...
static int resolveLocal(Compiler *compiler, Token *name){
    for(int i = compiler->localCount - 1; i >= 0; i--){
        ...
        if(identifiersEqual(name, &local->name)){
            if(local->depth == -1){
                error("Cannot read variable on its own initialization");
            }
            return i;
        }
    }
...
static void addLocal(Token name){
    if(current->localCount == UINT8_MAX + 1){
        error("Too many local variables on scope");
        return;
    }
...
static uint8_t argumentList(){
    ...
    if(!check(TOKEN_RIGHT_PAREN)){
        do {
            expression();
            if(argCount == 255){
                error("Cannot call with more than 255 arguments");
            }
            argCount++;
        }
...
static void returnStatement(){
    if(current->type == TYPE_SCRIPT){
        error("Cannot return form top-level code");
    }
